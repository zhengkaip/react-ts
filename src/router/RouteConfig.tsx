...
function renderRoutes({ routes }: IMenuListProps) {
    let list = [];
    const mapFunc = (R: RouteModel[]) =>
        R.map((route, i) => (
            <Route
                key={i}
                path={route.path}
                ...
                render={props => {
                    ...
                    if (!authed && route.auth) {
                        ...
                        const MyComponent = asyncComponent(() => import(`@/${route.component}`))
                        return (
                            <MyComponent {...props} route={route} />
                        )
                    }
                }}
            >
